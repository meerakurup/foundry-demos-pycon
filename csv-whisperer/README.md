...
A Streamlit data-analysis agent powered by **Microsoft Foundry** and the OpenAI Responses API with **Code Interpreter**. Pick a sample CSV, ask questions in plain English, and let the agent inspect the data, run pandas code, and generate charts.

**What it does:** Given one of the sample CSV datasets, CSV Whisperer summarizes the schema and answers follow-up questions with executable Python analysis. When a visual answer helps, it creates matplotlib charts inline in the Streamlit app.

### Key components

- **Streamlit app** (`app.py`) - chat-style UI with a sidebar dataset picker
...
Then choose a dataset from the sidebar and start asking questions.
...
1. The Streamlit sidebar loads a CSV from `data/`.
